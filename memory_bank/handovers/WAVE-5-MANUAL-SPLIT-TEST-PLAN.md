...
### 2.5 Local On‑Disk Models (Experimental)

We now support evaluating local ONNX/GGUF models via the `--adapter local` flag. This allows us to benchmark custom, domain‑specific models or even memory‑bank based retrieval systems (see Multi‑Expert Memory Bank strategy).

| Specification | Value |
|---------------|-------|
| **Provider** | Local (onnxruntime/gguf) |
| **Context Window** | Determined by model capacity |
| **Output Window** | N/A (model defines) |
| **Adapter** | `local` |
| **Usage** | `python -m split_test.main --adapter local --models <model_id>` |
| **Strengths** | Can test tuned models or memory bank retrieval engines |
| **Notes** | Must place `*.onnx`/`*.gguf` files under `$LOCAL_MODEL_DIR` or `models/` directory.

This mode paves the way for memory bank comparisons: treat each bank as a local model that returns knowledge snippets or synthesized responses.
---
