...
We have successfully transitioned the Omega Stack to a **16GB Physical / 24GB Total RAM** architecture. The `config.toml` is hardened (12GB Cap), the `SYSTEM_PROMPT_v2.5.md` is active, and the **Mnemosyne Migration** to a Zettelkasten structure is the primary directive for the next session.

## 2. Critical Artifacts (The "Keys")
| Artifact | Location | Purpose |
| :--- | :--- | :--- |
| **System Prompt** | `artifacts/haiku_pre_flight/SYSTEM_PROMPT_v2.5.md` | **CORE**. Must be loaded into Cloud Claude. |
| **Quickstart** | `artifacts/haiku_pre_flight/HAIKU_HANDOFF_QUICKSTART.md` | Step-by-step cloud initiation guide. |
| **Permissions** | `artifacts/haiku_pre_flight/PERMISSIONS_REMEDIATION_PROTOCOL_v2.0.md` | **NEW**. How to fix "Permission denied" errors. |
| **Migration** | `artifacts/haiku_pre_flight/MNEMOSYNE_AUDIT_REPORT.md` | The blueprint for the Zettelkasten refactor. |

## 3. Immediate Action Items (SESS-28)
1.  **Ignition**: Start the new session and load the `SYSTEM_PROMPT_v2.5.md`.
