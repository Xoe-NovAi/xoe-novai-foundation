...
- **Physical RAM**: 8 GB
- **Zram**: sizing now governed by conservative heuristic (repo templates; default `min(ram/2, 4GiB)`) — adjust after staging tests
- **Total Available Memory**: ~20 GB (8GB physical + configured zram)
- **Storage**: SSD (development environment)

### Memory Optimization
- **vm.swappiness**: recommended default `180` for zram-backed swap (improves responsiveness for in‑RAM swap)
- **Zram Algorithm**: preferred `zstd` (level=3) with `lz4` as low-CPU fallback
...
# Verify Zram
cat /proc/sys/vm/swappiness  # Recommended: 180 for zram-backed swap
zramctl
# node_exporter textfile metric
ls -la /var/lib/node_exporter/textfile_collector/xnai_zram.prom || true

```mermaid
flowchart LR
  K[Kernel zram] --> Z[zramctl]
