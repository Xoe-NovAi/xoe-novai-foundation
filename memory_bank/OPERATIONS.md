...
```bash
# Build all images (runtime and build base)
make build

# Build specific service using the builder image
# (uses Dockerfile.build -> xnai-base-build:latest)
docker build -t xnai/rag-api:latest -f Dockerfile.base .

# Build with buildkit cache
# avoid re-downloading Debian layers by leveraging cache; run from same directory
DOCKER_BUILDKIT=1 docker build --cache-from type=local -f Dockerfile.base .
# or explicitly disable pulling latest base
DOCKER_BUILDKIT=1 docker build --pull=false --cache-from type=local -f Dockerfile.base .

# Rebuild from scratch (clean)
make clean-images
make build

# Build and tag base image explicitly (ensure local availability for both podman and docker)
# This command builds xnai-base and xnai-base-build using both runtimes so
# docker-compose can succeed even if the system uses podman primarily.
make build-base

# Enforce image size budgets
make check-image-sizes      # invokes scripts/check_image_sizes.sh
```

> **Note**: All service Dockerfiles now follow a multi-stage builder/runtime pattern
> and use `xnai-base:latest` for runtime. The build tools live in `xnai-base-build`.

### Running Services
```bash
# Start all services (rebuild any images with build context; rag API will be built locally)
