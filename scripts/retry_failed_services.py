...
from knowledge.schemas.scraping_job_schema import (
    ScrapingJobSchema, JobStatus, JobPriority
)
...
class RetryExecutor:
    ...
    def print_summary(self):
        ...
        logger.info(f"Successful: {self.execution_stats['successful']}/{self.execution_stats['total_services']}")
        logger.info(f"Failed: {self.execution_stats['failed']}/{self.execution_stats['total_services']}")
        logger.info(f"Total content: {self.execution_stats['total_content_kb']:.2f} KB")
        
        for service in self.execution_stats['services']:
            if service['success']:
                logger.info(f"✓ {service['service']:25} {service['size_kb']:8.2f} KB  {service['files']:3} files")
            else:
