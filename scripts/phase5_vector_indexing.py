...
class DocumentChunker:
    ...
    def chunk_all_documents(self, docs_dir: str = "knowledge/technical_manuals") -> List[Dict]:
...
class VectorIndexer:
    ...
    def embed_chunks(self, chunks: List[Dict]) -> List[Tuple[str, List[float]]]:
        ...
        for i in range(0, len(contents), self.batch_size):
            ...
            if (i // self.batch_size + 1) % 5 == 0:
                logger.info(f"  - Embedded {min(i+self.batch_size, len(contents))}/{len(contents)} chunks")
    ...
    def search(self, query: str, collection_name: str = "technical_manuals", top_k: int = 10) -> List[Dict]:
    ...
    try:
        ...
        for query in test_queries:
            ...
            for i, result in enumerate(results, 1):
                logger.info(f"  {i}. [{result['service']}] Score: {result['score']:.3f}")
