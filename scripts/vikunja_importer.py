...
import argparse
import anyio
import asyncio  # Keep for TimeoutError exception type
import aiohttp
import sys
import os
...
class VikunjaImporter:
    ...
    async def get_or_create_label(self, label_name: str) -> Dict[str, Any]:
        ...
        label_data = {"name": label_name, "color": self._generate_color(label_name)}
    ...
    async def process_batch(self, tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # Create tasks concurrently using anyio.TaskGroup
        successful_results = []
        exceptions = []

        async def create_and_collect(task_data):
            try:
                result = await self.create_task(task_data)
                successful_results.append(result)
            except Exception as e:
                exceptions.append((task_data, e))

        async with anyio.create_task_group() as tg:
            for task in tasks_with_labels:
                tg.start_soon(create_and_collect, task)

        for task_data, exc in exceptions:
            print(f"Warning: Failed to create task: {exc}")

        return successful_results

    async def import_tasks(
        self, tasks: List[Dict[str, Any]], dry_run: bool = False
    ) -> Dict[str, Any]:
...
async def main():
    ...
    parser.add_argument(
        "--api-url",
        default="http://localhost:3456/api/v1",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Perform dry run without creating tasks"
    )
    ...
    if not token:
        if "VIKUNJA_API_TOKEN" in os.environ:
            ...
            print(
                "Error: API token required. Use --token or set VIKUNJA_API_TOKEN environment variable"
            )
            sys.exit(1)
