...
class AutonomousScrapingExecutor:
    ...
    def print_summary(self):
        ...
        logger.info(f"Duration: {self.execution_stats.get('duration_seconds', 0):.1f} seconds")
        ...
        for service in self.execution_stats['services']:
            if service.get('success', False):
                logger.info(f"✓ {service['service']:25} {service.get('size_kb', 0):8.2f} KB  {service.get('files', 0):3} files")
            else:
