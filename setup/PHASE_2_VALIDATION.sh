...
# Test 1: Verify runtime base image has audio dependencies and no build tools
echo ""
echo "[TEST 1] Verifying audio dependencies in xnai-base:latest (runtime)..."
docker run --rm xnai-base:latest bash -c "
  ...
  ! dpkg -l | grep build-essential && echo '  ✓ build-essential absent (runtime)'
" || exit 1

echo ""
echo "[TEST 1b] Verifying build base image includes build tools..."
docker run --rm xnai-base-build:latest bash -c "
  dpkg -l | grep build-essential && echo '  ✓ build-essential present in build image'
" || echo "  ⚠️ xnai-base-build may not exist or lack build tools"
