...
class HealthMonitor:
    ...
    def __init__(self, check_interval: float = 30.0):
        ...
        logger.info(f"Health Monitor initialized with check interval: {check_interval}s")
    ...
    async def _handle_state_transition(self, service_name: str, old_status: HealthStatus, result: HealthCheckResult):
        ...
        if old_status != new_status:
            ...
            if service_name in self._health_callbacks:
                try:
                    await self._health_callbacks[service_name](service_name, old_status, new_status, result)
                except Exception as e:
    ...
    def get_health_status(self, service_name: Optional[str] = None) -> Union[ServiceHealth, Dict[str, ServiceHealth]]:
    ...
    async def trigger_manual_check(self, service_name: str) -> Optional[HealthCheckResult]:
...
def create_rag_api_health_checker(base_url: str = "http://localhost:8000") -> HTTPHealthChecker:
...
def create_chainlit_ui_health_checker(base_url: str = "http://localhost:8001") -> HTTPHealthChecker:
...
def create_vikunja_health_checker(base_url: str = "http://localhost:3456") -> HTTPHealthChecker:
...
def create_caddy_health_checker(base_url: str = "http://localhost:8000") -> HTTPHealthChecker:
...
async def example_usage():
    ...
    # Add health callback
    async def health_callback(service_name: str, old_status: HealthStatus, new_status: HealthStatus, result: HealthCheckResult):
        logger.info(f"Health change for {service_name}: {old_status.value} -> {new_status.value}")
