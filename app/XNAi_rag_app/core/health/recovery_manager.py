...
class RecoveryManager:
    ...
    def add_recovery_rule(self, rule: RecoveryRule):
        ...
        logger.info(f"Added recovery rule for {rule.service_name}: {rule.recovery_action.value}")
    ...
    async def _execute_recovery_action(self, rule: RecoveryRule, health_result: HealthCheckResult):
        ...
        async with self._recovery_locks[service_name]:
            logger.info(f"Starting recovery for {service_name}: {rule.recovery_action.value}")
                ...
                try:
                    ...
                    if success:
                        ...
                        if service_name in self._recovery_callbacks:
                            try:
                                await self._recovery_callbacks[service_name](service_name, rule, recovery_attempt)
                            except Exception as e:
                                logger.error(f"Recovery callback failed for {service_name}: {e}")
                        
                        break
                    else:
                        logger.warning(f"Recovery attempt {attempt + 1} failed for {service_name}")
    ...
    async def _perform_recovery_action(self, rule: RecoveryRule, health_result: HealthCheckResult) -> bool:
        ...
        try:
            ...
            if rule.recovery_action == RecoveryAction.RESTART_SERVICE:
                ...
                return await self._request_manual_intervention(service_name, health_result)
            
            return False
            
        except Exception as e:
            logger.error(f"Recovery action {rule.recovery_action.value} failed for {service_name}: {e}")
            return False
    ...
    async def _restart_service(self, service_name: str) -> bool:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                logger.warning(f"Podman restart failed for {service_name}, trying Docker")
                ...
                if process.returncode == 0:
                    logger.info(f"Service {service_name} restarted successfully with Docker")
                    return True
                else:
    ...
    async def _request_manual_intervention(self, service_name: str, health_result: HealthCheckResult) -> bool:
    ...
    def get_recovery_history(self, service_name: Optional[str] = None) -> List[RecoveryAttempt]:
    ...
    def get_recovery_stats(self, service_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        for attempt in history:
            ...
            stats["actions_by_type"][action_name] = stats["actions_by_type"].get(action_name, 0) + 1
...
async def example_usage():
    ...
    async def recovery_callback(service_name: str, rule: RecoveryRule, attempt: RecoveryAttempt):
