...
class EnhancedHealthChecker(HealthChecker):
    ...
    async def _perform_enhanced_health_checks(self):
        ...
        for target in targets:
            try:
                ...
                # Update metrics
                await self._update_service_metrics(target, is_healthy, response_time, details)
                
                # Update Prometheus metrics
                self._update_prometheus_metrics(target, is_healthy, response_time, details)
                
            except Exception as e:
                logger.error(f"Health check failed for {target}: {e}")
                await self._handle_health_check_error(target, str(e))
    
    async def _check_service_health(self, service_name: str) -> tuple[bool, Dict[str, Any]]:
    ...
    async def _check_llm_health(self, details: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if torch.cuda.is_available():
                details['gpu_memory'] = torch.cuda.memory_allocated()
                details['gpu_memory_cached'] = torch.cuda.memory_reserved()
    ...
    async def _check_http_health(self, service_name: str, details: Dict[str, Any]) -> bool:
        """Generic HTTP health check"""
        try:
            import httpx
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(f'http://{service_name}:8000/health', timeout=5.0)
    ...
    async def _update_service_metrics(
        ...
        avg_response_time = sum(self._response_times[service_name]) / len(self._response_times[service_name])
        error_rate = self._error_counts[service_name] / max(1, len(self._response_times[service_name]))
        
        # Determine health status
        if not is_healthy:
            status = 'unhealthy'
        elif response_time > self._response_time_threshold or error_rate > self._error_rate_threshold:
            status = 'degraded'
        else:
            status = 'healthy'
        ...
        health.uptime = time.time() - self._service_start_times.get(service_name, time.time())
        ...
        try:
            ...
            try:
                ...
                for circuit_name, status_data in circuit_status.items():
                    ...
                    CIRCUIT_BREAKER_FAILURES.labels(circuit_name=circuit_name).set(status_data['fail_count'])
            except Exception as e:
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        unhealthy_services = [s for s, h in self.service_health.items() if h.status != 'healthy']
        if unhealthy_services:
            summary['overall_status'] = 'unhealthy' if any(h.status == 'unhealthy' for h in self.service_health.values()) else 'degraded'
