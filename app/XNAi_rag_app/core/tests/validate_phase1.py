...
def create_inmemory_circuit_breaker(
    name, failure_threshold=3, recovery_timeout=1, timeout=0.5
):
...
class Phase1Validator:
    ...
    async def test_graceful_degradation(self):
        ...
        try:
            ...
            degradation_manager.add_service_strategy(
                "test_service", FallbackStrategy(fallback_func), priority=10
            )
    ...
    async def test_end_to_end_integration(self):
        ...
        try:
            ...
            result = await degradation_manager.call_service(
                "integrated_service", service_func
            )
    ...
    async def test_real_world_scenarios(self):
        ...
        try:
            ...
            llm_circuit_breaker = create_inmemory_circuit_breaker(
                name="llm_service", failure_threshold=3, recovery_timeout=2, timeout=10
            )

            from core.circuit_breakers import CircuitBreakerStrategy

            llm_manager.add_service_strategy(
                "llm_service", CircuitBreakerStrategy(llm_circuit_breaker), priority=10
            )
...
async def main():
    ...
    success_rate = (
        (results["tests_passed"] / results["total_tests"] * 100)
