...
# Try to import LightRAG Client
try:
    from app.XNAi_rag_app.core.infrastructure.lightrag_client import (
        LightRAGClient,
        LightRAGConfig,
        LIGHTRAG_AVAILABLE,
    )
...
class KnowledgeConfig:
    ...
    lightrag_working_dir: str = "./storage/data/lightrag"
...
class KnowledgeClient:
    ...
    async def initialize(self) -> bool:
        ...
        # Try LightRAG (Hybrid Knowledge Graph)
        if self.config.lightrag_enabled and LIGHTRAG_AVAILABLE:
    ...
    async def _init_lightrag(self) -> bool:
        ...
        try:
            config = LightRAGConfig(
                ...
                mode=self.config.lightrag_mode,
            )
            self._lightrag_client = LightRAGClient(config)
            success = await self._lightrag_client.initialize()
            if success:
                self._use_lightrag = True
                logger.info("LightRAG (Hybrid Graph) initialized")
            return success
        except Exception as e:
    ...
    async def _init_embeddings(self) -> bool:
        ...
        try:
            # Domain-specific BERT Strategy (Ancient Greek, Philosophy, etc.)
            domain = getattr(self.config, "domain", "general")
            model_name = self.config.embedding_model

            if domain == "ancient-greek":
                model_name = "pranaydeeps/Ancient-Greek-BERT"
                logger.info("Using specialized Ancient-Greek-BERT for classical works")
            elif domain == "philosophy":
                model_name = "bowphs/SPhilBerta"
                logger.info("Using specialized Philosophy BERT (SPhilBerta)")
            ...
            try:
                ...
                # ValueError might occur if fastembed doesn't support the specific BERT model yet
                pass
            ...
            try:
                ...
                self._use_embeddings = True
                logger.info(
                    f"SentenceTransformer initialized with model: {model_name}"
                )
                return True
            except ImportError:
                pass
    ...
    async def search(
        ...
        # Use structured concurrency for parallel retrieval
        async with anyio.create_task_group() as tg:
            # Task 1: Qdrant Search
            if self._use_qdrant:
                tg.start_soon(self._search_qdrant_task, query, top_k, score_threshold, filters, results)

            # Task 2: LightRAG Search
            if self._use_lightrag:
                tg.start_soon(self._graph_query_task, query, results)

        # Fall back to FAISS if no results from primary backends
    ...
    async def _search_qdrant_task(self, query, top_k, score_threshold, filters, results_list):
        res = await self._search_qdrant(query, top_k, score_threshold, filters)
        results_list.extend(res)

    async def _graph_query_task(self, query, results_list):
        graph_context = await self.graph_query(query)
        if graph_context:
            results_list.append(
                SearchResult(
                    id="graph_context",
                    ...
                    score=0.9,
                    source="lightrag",
                    metadata={"type": "relational_context"},
                )
            )

    async def graph_query(self, query: str, mode: Optional[str] = None) -> str:
        """Perform a relational knowledge graph query."""
        if not self._use_lightrag or not self._lightrag_client:
            return ""
        return await self._lightrag_client.query(query, mode=mode)
