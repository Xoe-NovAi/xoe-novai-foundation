...
class ServiceOrchestrator:
    ...
    def __init__(self, enable_openpipe: bool = True):
        ...
        # AnyIO Lock for structured concurrency compliance
        self._llm_init_lock = anyio.Lock()
    ...
    async def initialize_all(self) -> Dict[str, Any]:
        ...
        try:
            ...
            redis_port = int(self.config.get('redis', {}).get('port', 6379))
            tls_enabled = os.getenv("REDIS_TLS_ENABLED", "false").lower() == "true"
            scheme = "rediss" if tls_enabled else "redis"
            try:
                await initialize_circuit_breakers(host=redis_host, port=redis_port, password=redis_pass)
                await initialize_voice_circuit_breakers(host=redis_host, port=redis_port, password=redis_pass)
            ...
            async with anyio.create_task_group() as tg:
                # We only wait for models to be PRIMED if necessary, 
                # but here we want fast startup.
                tg.start_soon(self._background_init_models)
                # We DON'T start infinite loops here.
            ...
            if voice_enabled:
                ...
                logger.info("ℹ Voice Interface disabled in config")
                self.services['voice'] = None
