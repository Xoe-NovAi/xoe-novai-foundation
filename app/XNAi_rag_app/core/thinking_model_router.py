...
class ModelVariant(Enum):
    ...
    REGULAR = "regular"
    THINKING = "thinking"
    ESCALATION_TINY = "escalation_tiny"
    ESCALATION_HYBRID = "escalation_hybrid"
    ESCALATION_DEEP = "escalation_deep"
...
class RoutingDecision:
    ...
    escalation_path: List[ModelVariant] = None
    domain: str = "general"
    assigned_personas: List[str] = None

class ThinkingModelRouter:
    """
    Routes tasks to optimal model variant with expert personas.
    """
    
    # Domain-specific model registry
    MODEL_REGISTRY = {
        "code": {
            ModelVariant.ESCALATION_TINY: "storage/models/deepseek-coder-1.3b.gguf",
            ModelVariant.ESCALATION_AUTHORITY: "storage/models/deepseek-coder-33b.gguf"
        },
        "research": {
            ModelVariant.ESCALATION_TINY: "storage/models/Llama-3.1-FastDraft-150M.gguf",
            ModelVariant.ESCALATION_AUTHORITY: "storage/models/Krikri-8b-Instruct.gguf"
        }
    }

    # Persona templates for 3-turn triangulation
    PERSONA_TEMPLATES = ["SKEPTIC", "ARCHITECT", "SYNTHESIZER"]
    ...
    # Escalation Path Models
    ESCALATION_MODELS = {
        ModelVariant.ESCALATION_TINY: "storage/models/Llama-3.1-FastDraft-150M.gguf",
        ModelVariant.ESCALATION_HYBRID: "storage/models/Llama-3.2-1B-Instruct.gguf",
        ModelVariant.ESCALATION_DEEP: "storage/models/Mistral-7B-Instruct-v0.3.gguf",
        ModelVariant.ESCALATION_AUTHORITY: "storage/models/Krikri-8b-Instruct.gguf",
    }
    ...
    def route(
        ...
        Args:
            ...
            research_required: If True, triggers the escalation research chain
        ...
        # Special case: Research Escalation Path
        if research_required:
            escalation_path = [
                ModelVariant.ESCALATION_TINY,
                ModelVariant.ESCALATION_HYBRID,
                ModelVariant.ESCALATION_DEEP,
                ModelVariant.ESCALATION_AUTHORITY
            ]
            
            # Dynamic domain detection
            domain = "code" if "code" in task_type.lower() or "implement" in task_type.lower() else "research"
            
            # Select model from registry or default
            base_model = self.MODEL_REGISTRY.get(domain, {}).get(
                ModelVariant.ESCALATION_TINY, 
                self.ESCALATION_MODELS[ModelVariant.ESCALATION_TINY]
            )

            return RoutingDecision(
                model=base_model,
                variant=ModelVariant.ESCALATION_TINY,
                reason=f"Hierarchical escalation for domain: {domain}",
                estimated_latency_ms=6000, # Increased for triangulation
                estimated_tokens=8000,
                escalation_path=escalation_path,
                domain=domain,
                assigned_personas=self.PERSONA_TEMPLATES
            )
