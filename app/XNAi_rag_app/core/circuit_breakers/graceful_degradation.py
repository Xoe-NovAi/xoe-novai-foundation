...
class DegradationStrategy(ABC):
    ...
    @abstractmethod
    async def execute(
        self, func: Callable[[], Awaitable[T]], context: Dict[str, Any]
    ) -> T:
...
class CacheFirstStrategy(DegradationStrategy):
    ...
    async def execute(
        ...
        if (
            ...
            and current_time - self._last_cache_time < self.cache_timeout
        ):
            try:
...
class DegradationManager:
    ...
    def remove_strategy(self, strategy_name: str):
        ...
        self.strategies = [
            s for s in self.strategies if s[0].get_strategy_name() != strategy_name
        ]

    async def execute_with_degradation(
        self, func: Callable[[], Awaitable[T]], context: Optional[Dict[str, Any]] = None
    ) -> T:
    ...
    def get_strategy_status(self) -> List[Dict[str, Any]]:
        ...
        status = []
        for strategy, priority in self.strategies:
            strategy_info = {"name": strategy.get_strategy_name(), "priority": priority}

            # Add strategy-specific info
            if isinstance(strategy, CircuitBreakerStrategy):
                strategy_info["circuit_breaker_metrics"] = (
                    strategy.circuit_breaker.get_metrics()
                )
...
class ServiceDegradationManager:
    ...
    def add_service_strategy(
        self, service_name: str, strategy: DegradationStrategy, priority: int = 1
    ):
    ...
    def add_global_strategy(self, strategy: DegradationStrategy, priority: int = 1):
    ...
    async def call_service(
        ...
        if service_name in self.service_managers:
            try:
                return await self.service_managers[
                    service_name
                ].execute_with_degradation(func, context)
            except Exception as e:
                logger.warning(
                    f"Service-specific degradation failed for {service_name}: {e}"
                )
    ...
    def get_service_status(self, service_name: str) -> Dict[str, Any]:
        ...
        if service_name in self.service_managers:
            ...
            return {"service_name": service_name, "error": "Service not registered"}
...
def create_vector_search_degradation_manager() -> ServiceDegradationManager:
    ...
    async def degraded_response():
        return {
            "results": [],
            "degraded": True,
            "message": "Search temporarily limited",
        }
...
class IntegratedDegradationManager:
    ...
    def register_circuit_breaker_service(
        ...
        self.service_manager.add_service_strategy(
            service_name, cb_strategy, priority=10
        )
        ...
        if fallback_func:
            ...
            self.service_manager.add_service_strategy(
                service_name, fallback_strategy, priority=5
            )
...
async def example_usage():
    ...
    llm_config = CircuitBreakerConfig(
        failure_threshold=5, recovery_timeout=60, timeout=30, name="llm_service"
    )
    ...
    integrated_manager.register_circuit_breaker_service(
        "llm_service", llm_circuit_breaker, fallback_func=llm_fallback
    )
