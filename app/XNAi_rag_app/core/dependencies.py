...
try:
    ...
    logger.debug("fastapi not available - Request/Depends type hints will be disabled")
...
async def get_embeddings_func():
    """
    Returns an async function that takes a string and returns a list of floats.
    Used by LightRAG for incremental indexing.
    """
    embeddings_model = await get_embeddings_async()
    
    async def embed_func(text: str) -> List[float]:
        # Wrap the sync call in a thread
        result = await anyio.to_thread.run_sync(embeddings_model.embed_query, text)
        return [float(x) for x in result]
        
    return embed_func

async def get_llm_complete(prompt: str, **kwargs) -> str:
    """
    Get a non-streaming LLM completion.
    Used for LightRAG and high-fidelity extraction.
    """
    llm = await get_llm_async(**kwargs)
    # LangChain's invoke method provides standard completion
    response = await anyio.to_thread.run_sync(llm.invoke, prompt)
    return str(response)
