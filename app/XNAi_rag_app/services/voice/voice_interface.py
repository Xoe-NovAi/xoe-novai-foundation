...
from ...core.circuit_breakers import voice_stt_breaker, voice_tts_breaker, get_circuit_breaker_status
...
class VoiceMetrics:
    ...
    def _init_metrics(self):
        ...
        self.stt_requests_total = Counter(
            "xoe_voice_stt_requests_total", "Total STT transcription requests", ["status", "provider"], registry=self._registry
        )

        self.tts_requests_total = Counter(
            "xoe_voice_tts_requests_total", "Total TTS synthesis requests", ["status", "provider"], registry=self._registry
        )

        self.wake_word_detections_total = Counter(
            "xoe_voice_wake_word_detections_total", "Total wake word detections", ["status"], registry=self._registry
        )

        self.rate_limit_exceeded_total = Counter(
            "xoe_voice_rate_limit_exceeded_total", "Total rate limit exceeded events", ["client_id"], registry=self._registry
        )
        ...
        self.audio_input_level = Gauge(
            "xoe_voice_audio_input_level", "Current audio input level (0-1)", registry=self._registry
        )

        self.stt_model_loaded = Gauge(
            "xoe_voice_stt_model_loaded", "Whether STT model is loaded", ["provider"], registry=self._registry
        )

        self.tts_model_loaded = Gauge(
            "xoe_voice_tts_model_loaded", "Whether TTS model is loaded", ["provider"], registry=self._registry
        )

        self.circuit_breaker_open = Gauge(
            "xoe_voice_circuit_breaker_open", "Whether circuit breaker is open", ["component"], registry=self._registry
        )

        self.voice_info = Info("xoe_voice", "Voice subsystem configuration", registry=self._registry)
...
class STTProvider(str, Enum):
    ...
    SAMBANOVA_WHISPER = "sambanova_whisper"
...
class VoiceSessionManager:
    ...
    def save_session(self) -> bool:
        ...
        try:
            ...
            self._redis_client.setex(session_key, self.SESSION_TTL, json.dumps(self._session_data, default=str))
            return True
        except Exception as e:
...
class AudioStreamProcessor:
    ...
    def __init__(self, config: VoiceConfig):
        ...
        self._cleanup_tracker = weakref.finalize(self, self._finalize_callback, self.config)
    ...
    def __del__(self):
        ...
        if hasattr(self, "_audio_chunks") and self._audio_chunks:
            logger.warning("AudioStreamProcessor destroyed without explicit cleanup", extra={"buffer_size": self._total_bytes})
            self.cleanup()
...
class VoiceInterface:
    def __init__(self, config: Optional[VoiceConfig] = None):
        ...
        logger.info(
            f"VoiceInterface initialized with offline_mode={self.config.offline_mode}, preload_models={self.config.preload_models}"
        )
        ...
        # SambaNova Multi-Account Rotation
        self._sambanova_keys = self._load_sambanova_keys()
        self._sambanova_key_index = 0

        self.loaded_tier = None
        self._initialize_models()

    def _load_sambanova_keys(self) -> List[str]:
        """Load up to 8 SambaNova API keys from environment."""
        keys = []
        for i in range(1, 9):
        ...
        if not keys and os.getenv("SAMBANOVA_API_KEY"):
            keys.append(os.getenv("SAMBANOVA_API_KEY"))
            
        return keys

    def _get_next_sambanova_key(self) -> str:
        """Get the next SambaNova API key in rotation."""
        if not self._sambanova_keys:
            return ""
        key = self._sambanova_keys[self._sambanova_key_index]
        self._sambanova_key_index = (self._sambanova_key_index + 1) % len(self._sambanova_keys)
        return key
        ...
        try:
            ...
            if self.config.stt_provider == STTProvider.FASTER_WHISPER and FASTER_WHISPER_AVAILABLE:
                ...
                if self.stt_model is not None:
                    logger.info(
                        f"Unloading model {self.loaded_model_name} to load {selected_model_name} (Tier {current_tier})..."
                    )
                    del self.stt_model
                    self.stt_model = None
                    gc.collect()
    ...
    async def transcribe_audio(self, audio_data: bytes, audio_format: str = "wav") -> Tuple[str, float]:
        ...
        # SambaNova Whisper (API-based with rotation)
        if self.config.stt_provider == STTProvider.SAMBANOVA_WHISPER:
            return await self._transcribe_sambanova(audio_data)
        ...
        try:
            if hasattr(asyncio, "timeout"):
                ...
                segments, info = await asyncio.wait_for(
                    self._transcribe_impl(audio_file), timeout=self.config.stt_timeout_seconds
                )
        ...
        if self.metrics["total_transcriptions"] > 1:
            self.metrics["avg_stt_latency_ms"] = (
                self.metrics["avg_stt_latency_ms"] * (self.metrics["total_transcriptions"] - 1) + latency * 1000
    ...
    async def _transcribe_sambanova(self, audio_data: bytes) -> Tuple[str, float]:
        """Transcribe audio using SambaNova Whisper API with 8-account rotation."""
        import httpx
        key = self._get_next_sambanova_key()
        if not key:
            logger.warning("No SambaNova API key available for STT")
            return "[No SambaNova API key available]", 0.0
        ...
        # Use filename appropriate for the format
        files = {
        ...
        t0 = time.time()
        try:
            async with httpx.AsyncClient(timeout=self.config.stt_timeout_seconds) as client:
                ...
                if "voice_metrics" in globals() and voice_metrics:
                    voice_metrics.record_stt_request("success", "sambanova", latency)
                    
                logger.info(f"SambaNova STT complete (Key Rotation Active): {latency:.2f}s")
                return transcription, 0.99
                
        except Exception as e:
            logger.error(f"SambaNova STT failed: {e}")
            if "voice_metrics" in globals() and voice_metrics:
                voice_metrics.record_stt_request("error", "sambanova", 0)
            return f"[SambaNova STT error]", 0.0
        ...
        try:
            ...
            if self.metrics["total_voice_outputs"] > 1:
                self.metrics["avg_tts_latency_ms"] = (
                    self.metrics["avg_tts_latency_ms"] * (self.metrics["total_voice_outputs"] - 1) + latency * 1000
    ...
    def _initialize_debug_system(self):
        ...
        try:
            ...
            self.debug_metadata = {
                ...
                "recordings": [],
                "stats": {"human_voice_recordings": 0, "ai_voice_recordings": 0, "total_audio_mb": 0.0},
            }
