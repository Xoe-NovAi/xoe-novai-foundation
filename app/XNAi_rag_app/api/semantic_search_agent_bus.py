...
# Path resolution for Agent Bus communication and knowledge
INBOX_DIR = resolve_path("communication_hub/inbox")
OUTBOX_DIR = resolve_path("communication_hub/outbox")

# Knowledge sources
TECHNICAL_MANUALS_DIR = resolve_path("knowledge/technical_manuals")
...
class SemanticSearchAgentBusService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initialized {self.agent_name} with capabilities: {self.capabilities}"
        )
    ...
    async def listen_for_tasks(self, poll_interval: int = 5):
        ...
        """
        logger.info(
            f"Listening for tasks in {INBOX_DIR} (poll interval: {poll_interval}s)"
        )
    ...
    async def handle_task(self, message: Message):
        ...
        try:
            logger.info(
                f"Handling task {task_id}: {message.content.get('task_type', 'unknown')}"
            )
            ...
            logger.info(
                f"Task {task_id} completed successfully. Found {len(results)} results."
            )
