...
# Include all API routers with a unique prefix to avoid conflicts with Open WebUI
app.include_router(api_router, prefix="/xnai/api/v1")
...
async def on_startup():
    ...
    # Start Redis health monitoring if available
    from ..core.circuit_breakers import registry as cb_registry
    if cb_registry and cb_registry.redis_manager:
        import asyncio
        asyncio.create_task(cb_registry.redis_manager.start_monitoring())
    
    # Start degradation monitor in background
    import asyncio
    asyncio.create_task(degradation_manager.start_monitoring())
