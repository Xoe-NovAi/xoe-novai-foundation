...
## 🏙️ Metropolis Expert Contributions
The Omega Metropolis is an evolving network of domain experts. We welcome contributions that expand our "Expert Matrix":

1.  **New Technical Domains**: Propose new domains (e.g., `--security`, `--bioinformatics`) by defining the instance mapping in `scripts/xnai-gemini-dispatcher.sh`.
2.  **Expert Soul Templates**: Share Markdown templates for specific domain expertise in `expert-knowledge/souls/`.
3.  **Reflection Hooks**: Implement new "Self-Evolution" logic in `scripts/expert-soul-reflector.py`.
4.  **Multi-Provider Sub-domains**: Help us map new providers (e.g., DeepSeek, Grok) into the hierarchical hierarchy.

Before submitting a Metropolis-related PR, ensure you have run:
```bash
