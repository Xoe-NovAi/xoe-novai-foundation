# Stack-Cat v0.1.0-alpha User Guide

**Comprehensive Documentation for the Xoe-NovAi Foundation Stack Documentation Generator**
...
Stack-Cat is a comprehensive documentation generator designed specifically for the Xoe-NovAi Foundation stack. It automatically collects, organizes, and formats your codebase into multiple documentation formats, making it easy to:
...
- v0.1.0-alpha stack compliance validation
...
├── scripts/
│   └── stack-cat/
│       ├── stack-cat.sh              # Main script (v0.1.0-alpha)
...
Stack-Cat automatically manages documentation history with intelligent archiving:

```bash
# Automatic cleanup (runs every generation)
find stack-cat-output/ -maxdepth 1 -type d -mtime +7 -exec rm -rf {} \;
# Keeps last 7 days of snapshots

# Manual archive management
./stack-cat_v017.sh --archive-old 30  # Keep last 30 days
./stack-cat_v017.sh --cleanup         # Remove all but latest
```

**Retention Policy:**
- **Latest symlinks**: Always preserved
- **Daily snapshots**: 7-day retention (configurable)
- **Weekly archives**: 30-day retention
- **Monthly archives**: 90-day retention
...
# Daily at 2 AM (quiet mode)
0 2 * * * cd /path/to/stack-cat && ./stack-cat_v017.sh -g default -f json >/dev/null 2>&1

# Weekly full documentation (Sunday at 3 AM)
0 3 * * 0 cd /path/to/stack-cat && ./stack-cat_v017.sh -g default -f all >/dev/null 2>&1
```

### Git Integration

Automatic documentation on commits:
...
#!/bin/bash
cd scripts/stack-cat
./stack-cat_v017.sh -g default -f md >/dev/null 2>&1
EOF
...
Stack-Cat can integrate with monitoring systems:
...
# Extract metrics
PROJECT_SIZE=$(jq '.statistics.total_size_bytes' stack-cat-manifest_latest.json)
FILE_COUNT=$(jq '.statistics.file_count' stack-cat-manifest_latest.json)

# Send to monitoring
curl -X POST http://prometheus-pushgateway:9091/metrics/job/stack-cat \
  -d "project_size_bytes $PROJECT_SIZE
...
```bash
# Backup script
cat > backup-stack-cat.sh <<'EOF'
#!/bin/bash
DATE=$(date +%Y%m%d)
BACKUP_DIR="/backups/stack-cat/$DATE"

mkdir -p "$BACKUP_DIR"
cp -r stack-cat-output/* "$BACKUP_DIR/"
cp whitelist.json groups.json "$BACKUP_DIR/"

# Compress and upload
tar czf "$BACKUP_DIR.tar.gz" -C /backups/stack-cat "$DATE"
# Upload to S3/cloud storage here

# Cleanup old backups (keep last 30)
find /backups/stack-cat -maxdepth 1 -type d -mtime +30 -exec rm -rf {} \;
EOF

# Run weekly
0 4 * * 0 /path/to/backup-stack-cat.sh
```
...
cat > check-docs-quality.sh <<'EOF'
#!/bin/bash
echo "Checking documentation quality..."

# Check file sizes
find stack-cat-output/ -name "*.md" -size +10M -exec ls -lh {} \;

# Check for sensitive data
grep -r "password\|secret\|token" stack-cat-output/ || echo "No sensitive data found"

# Validate JSON structure
jq . stack-cat-manifest_latest.json >/dev/null && echo "JSON valid"

# Check symlink integrity
ls -la *_latest.* | grep -v "->"
EOF

# Run after each generation
./check-docs-quality.sh
```
