...
| **Memory** | <6GB | 5.2GB peak | podman stats |
...
**Validation:**
```bash
podman exec xnai_rag_api python3 -c "from config_loader import load_config; print('✅ OK')"
```
...
**Validation:**
```bash
podman exec xnai_rag_api python3 -c "from dependencies import get_llm; llm = get_llm(); print('✅ Loaded')"
```
...
**Validation:**
```bash
podman exec xnai_rag_api python3 -c "from circuitbreaker import circuit; print('✅ Circuit breaker available')"
```
...
|---------|----------|---------|
| CHAINLIT_NO_TELEMETRY=true | .env, Podmanfile.chainlit | Disable Chainlit analytics |
| CRAWL4AI_TELEMETRY=0 | .env, Podmanfile.crawl | Disable CrawlModule telemetry |
...
**Stage 3: Podman Build (Zero-Network Verified)**
```bash
podman build --build-arg OFFLINE_BUILD=true \
  --network=none \
  -f Podmanfile.api \
  -t xnai_api:latest .
...
Ref: xnai-group3-artifact1-docker-deployment.md (Podman architecture).
...
| **Memory** | <6GB | 5.2GB peak | podman stats | During 10k-vector ingestion |
...
# 2. Isolation
podman stop xnai_rag_api
podman logs xnai_rag_api > /backups/rag_crash_$(date +%s).log
...
# Vulkan BUILD configuration (Podmanfile.api)
...
podman inspect xnai_rag_api | grep -E "User|CapDrop"
```
...
release:
  ...
  script:
    ...
    - podman build --network=none -f Podmanfile.api .
...
```json
{
  ...
  "next_steps": [
    ...
    "Update Vulkan CMake in Podmanfile.api (Phase 2 prep)",
