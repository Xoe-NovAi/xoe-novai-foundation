# Podman Optimization - Code Changes Reference
...
### Files: `Podmanfile.api`, `Podmanfile.chainlit`, `Podmanfile.crawl`
...
podman build -f Podmanfile.api -t test:with-cleanup .
podman history test:with-cleanup | grep "Site-packages cleanup"
...
# 3. Build images
podman build -f Podmanfile.api -t xoe-test:api .
podman build -f Podmanfile.chainlit -t xoe-test:ui .
podman build -f Podmanfile.crawl -t xoe-test:crawler .

# 4. Check sizes
podman images xoe-test:* --format "table {{.Repository}}\t{{.Size}}"

# 5. Test imports
podman run --rm xoe-test:api python -c "from faster_whisper import WhisperModel; print('✓')"
podman run --rm xoe-test:ui python -c "try:
    import torch
except ImportError:
    print('✓ PyTorch correctly removed')"

# 6. Test functionality
podman run --rm xoe-test:api python -c "
...
git checkout requirements-api.txt requirements-chainlit.txt requirements-crawl.txt Podmanfile.*
...
```
docs: Podman optimization - 60% image size reduction
...
| `Podmanfile.api` | Add site-packages cleanup | 100MB saved | 🟡 Medium |
| `Podmanfile.chainlit` | Add site-packages cleanup | 50MB saved | 🟡 Medium |
| `Podmanfile.crawl` | Add site-packages cleanup + optional trafilatura | 400-600MB saved | 🟡 Medium |
...
- **Import errors**: Check package installed with `podman run image pip show package-name`
- **Build failures**: Use `podman build --progress=plain` for detailed logs
- **Testing issues**: Run `podman run -it image bash` for interactive debugging
- **Size verification**: Use `podman history image-name` or `dive image-name`
